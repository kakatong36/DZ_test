		...
		<div class="scroller-outer" :class='{transprant:isReady}' >
			<div class="scroll-wrapper"  >
				<ul class="scroll-list clearfix">
					<li class="" v-for='(suit,index) in suitList' @click='go_next(suit)'>
						<div class="scroll-item">
							<a href="javascript:;">
								<div class="poster">
									<img :src="imgUrl+loginer.SelUser.SellerId+'/'+imgModel+'/image/'+suit.SuitPic" />
								</div>
								<h4>{{suit.Name}}</h4>
								<h5>￥{{suit.Price}}</h5>
		...
		data() {
			...
			created() {
				...
				if(sign == 'love') {
					...
					this.requestUrl  = '/style_api_PlatCustomSuit_GetSuitList';
				} else if(sign == 'honey') {
					this.$store.state.header.title = '守护甜心';
					this.requestUrl  = '/style_api_PlatCustomSuit_GetGuardSuitList';
				} else {
					this.$store.state.header.title = '我的新娘'
				};
				this.loginer = JSON.parse(localStorage.getItem("user"));
			},
			mounted() {
			...
			/*换一批*/
				...
				go_next(suit){
					...
					switch(this.sign){
						case 'love':
						path = '/love/loveDetails/'+suit.ID;
						break;
						case 'honey':
							path = '/hoeny/honeyDetails/'+suit.ID;
						break;	
						default :
						    path = "/bride"
					}
