					...
					<li class="" v-for='(suit,index) in suitList' @click='go_next(suit)'>
						<div class="scroll-item">
							<a href="javascript:;">
								...
								<div class="params">
									<span v-for='attr in suit.SkuAttrList'><b>{{attr.StyleAttrValueName}}</b><br>{{attr.StyleAttrName}}</span>
								</div>
							</a>
						</div>
					</li>
	...
	export default {
		data() {
			...
			created() {
                ...
                this.requestUrl  = '/purchase_Api_PersonalityStyle_GetWomanGuardSweetList'; //只要女款的
			...
			/*换一批*/
				...
				go_next(suit){
					var path = '/hoeny/honeyDetails/'+suit.Id;
					this.$router.push({
						path:path
					})
				},
