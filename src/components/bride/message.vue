		...
		<div class="left " :class="{honey:sign == 'honey'}">
				...
				<div class="img-group">
					<a>
						<div class="img-item" style="position:relative;">
							<span class='Lettering' style="position:absolute;top:80px;left:0;right:0;color:#aaa;text-align:center;">{{personal_order.DetailsList[0].Lettering}}</span>
			...
			<div class="bride-content">
				...
				<div class="img-group">
					<a>
						<div class="img-item" style="position:relative;">
							<span class='Lettering' style="position:absolute;top:80px;left:0;right:0;color:#aaa;text-align:center;">{{personal_order.DetailsList[0].Femalelettering}}</span>
							<img src="../../common/images/personal/copy@3x.png" />
						</div>
						<p>女款刻字示例</p>
					</a>
			...
			<div class="honey-content" v-if='sign == "honey"'>
				<div class="main-img">
					...
					<div class="img-item">
						<span class='Lettering'>{{personal_order.DetailsList[0].Femalelettering}}</span>
						<img src="../../common/images/personal/copy@3x.png" />
					</div>
				</div>
				<div class="main-img">
		...
		<div class="right">
			<div class="inner">
				...
				<div class="main-box" v-show="!next">
					<div class="info-box effect" v-if='sign != "bride"' style=" display:none">
						<h2>男方</h2>
						<div class="list-box">
							<div class="tab-content list" style="width: 290px;">
								<div class="form-group">
									<label>姓名</label>
									<input type="text" name="Name" v-model='manAdditional.Consignee' class="require" data-tips='请填写女方姓名'/>
								</div>
								<div class="form-group">
									<label>手寸</label> 
									<input type="tel" name="Hand" v-model='manAdditional.Hand' class="require" data-tips='请填写女方手寸'/>
								</div>
								<div class="form-group">
									<label>刻字</label>
					...
					<div class="info-box effect" style="transition:all .8s .1s linear;-webkit-transition:all .8s .1s linear;">
						...
						<div class="list-box">
							<div class="tab-content list" style="width: 290px;">
								<div class="form-group">
									<label>姓名</label>
									<input type="text" name="name" v-model='personal_order.Consignee' class="require" data-tips='请填写女方姓名'/>
								</div>
								<div class="form-group">
									<label>刻字</label>
									<input type="text" name="Lettering" v-model='personal_order.DetailsList[0].Femalelettering' maxlength="8"/>
								</div>
								<div class="form-group">
									<label>字体</label>
									<select name="Typeface" v-model='personal_order.DetailsList[0].Typeface' style="width:100px;height:28px;border-color:#999;font-size:12px;line-height:1em;">
										<option value="新宋体" >新宋体</option>
										<option value="微软雅黑">微软雅黑</option>
										<option value="篆体">篆体</option>
									</select>
...
export default {
    ...
    created(){
		let obj = JSON.parse(localStorage.getItem('RingObj'));
		this.imgMainUrl = obj.getRingpictureList.ImagesList[0].ImageCode;
    },
    mounted(){
    ...
    methods:{
    	...
    	set_devier(num){
    		this.tab = num;
    		if(this.tab == 0){
				// this.personal_order.DeliverGoodsId ='快递寄送服务';
				this.personal_order.DeliverGoodsName ='快递寄送服务';
    		}else{
    			// this.personal_order.DeliverGoodsId  = '上门自取';
        ...
        set_province(obj){
            ...
            set_city(obj){
                this.personal_order.CityName = obj.code ? obj.value : '';
    	...
    	create_order(){
    		this.loading();
			let attr = JSON.parse(localStorage.getItem('RingObj'));
			let attrList = [];
			attr.getRingparametersList.forEach(item => {
				let val = item.AttrValueList[0];
				attrList.push({
					...
					DictionaryValueId: val.DictionaryValueId
				})
			});

			let detailsList = [{
				GroupId: attr.GroudId,
				GroupName: attr.getRingpictureList.GroupName,
				Typeface: '' ,
				Femalelettering:this.personal_order.DetailsList[0].Femalelettering,
				GoldPrice: 0,
				StonePrice: 0,
				TotalPrice: attr.TotalPrice,
				Fineness: attr.getRingparametersList[1].first,
				ImgUrl: '',
				Name: '',
				AttrList: attrList
			}];
			this.personal_order.DetailsList = detailsList;
				this.personal_order.CustomizeType = 3;
                this.personal_order.OrderTotal = this.CommodityPrice+this.personal_order.Freight + this.personal_order.InsurancePrice;
				this.personal_order.OrderTotal = 0;
                var options = {
                    url:"/order_Api_CustomizeOrder_CreateMyBrideOrder",
					params:this.personal_order
